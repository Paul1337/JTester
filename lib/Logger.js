...
class Logger {
    static printGlobalResult() {
        ...
        if (OK)
            console.log(colors[cl].underline.bold('All Ok'));
        // else console.log(colors[cl].underline(`Failed: ${globalTest.failed} tests`));
        else
            Logger.printFailedMicrotests();
    }
    ...
    static printMicroTestResult(microTest) {
        const itemInfo = microTest.value.result.description ??
            microTest.meta.description ??
            microTest.meta.index ??
            'no-descr';
        const resultMsg = `${microTest.meta.absoluteTitle} (${itemInfo}): ${colors.bold(microTest.value.result.resultTxt)}`;
        const messageFormatted = microTest.value.result.state === true ? colors.green(resultMsg) : colors.red(resultMsg);
        console.log(messageFormatted);
    }
    static printFailedMicrotests() {
        if (failedMicrotests.length === 0)
            return;
